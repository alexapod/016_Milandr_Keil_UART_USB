...
if (usb_data_received) {
    ...
    // Обработка команды (уже включает send_string)
    process_immediate_command(cmd);

    // Лёгкое подтверждение в USB (опционально)
    send_usb("OK\r\n");
}
if (auto_temp_enabled && (ms_ticks - last_auto_read) >= 3000) {
    ...
    send_string(msg);   // В UART
    send_usb(msg);      // И в USB
